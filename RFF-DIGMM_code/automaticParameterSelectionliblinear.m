function [bestc, bestg, bestcv] = automaticParameterSelectionliblinear(trainLabel, trainData, Ncv, option)
...
% % %             cv = get_cv_ac(trainLabel, [(1:NTrain)' trainData*trainData'], cmd, Ncv);
           ...
           cmd = ['-s 2 -c ', num2str(2^log2c)];
            cv = get_cv_acliblinear(trainLabel, trainData, cmd, Ncv);
            if (cv >= bestcv),
