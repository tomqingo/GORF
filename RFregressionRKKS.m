function [mseTrain, mseTest,outTest] = RFregressionRKKS(Z_train,Z_test,Y_train,Y_test, type,flagCV)
...
%         Zt = conj(Z_test');
        ...
        libsvmparam1 = ['-s 11 -c ',num2str(C) ];
        svmodel = train(Y_train,Zx,libsvmparam1);
        
        [~,accuracyTr, ~] = predict( Y_train,  Zx, svmodel);
        [outTest,accuracyTe, ~] = predict( Y_test,  Zt, svmodel);
        mseTrain = sqrt(accuracyTr(2));
        mseTest = sqrt(accuracyTe(2));
end